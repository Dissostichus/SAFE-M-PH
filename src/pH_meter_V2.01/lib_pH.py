...
T9 = np.array([[0, 9.22], [5, 9.17], [10, 9.12], 
      [15, 9.08], [20, 9.04], [25, 9.00], 
      [30, 8.97], [35, 8.94], [40, 8.91], 
      [45, 8.88], [50, 8.85], [55, 8.83], 
      [60, 8.80], [65, 8.77], [70, 8.75], 
      [75, 8.72], [80, 8.68], [85, 8.65], 
      [90, 8.61], [95, 8.57]])
...
T11 = np.array([[0, 11.74], [5, 11.57], [10, 11.41], 
      [15, 11.28], [20, 11.14], [25, 11.00], 
      [30, 10.87], [35, 10.74], [40, 10.61], 
      [45, 10.48], [50, 10.35], [55, 10.24], 
      [60, 10.14], [65, 10.02], [70, 9.90], 
      [75, 9.78], [80, 9.69], [85, 9.59], 
      [90, 9.50], [95, 9.40]])
...
f9 = interpolate.interp1d(x_T9, y_T9)
f10 = interpolate.interp1d(x_T10, y_T10)
f11 = interpolate.interp1d(x_T11, y_T11)
...
def Calibration(calib3, buffer_value, buffers = [7, 4], n = 200, port_test = ''):
    ...
    for i in range(len(buffers)):
        ...
        if buffers[i] == 9:
            y=np.round(f9(temperature),2)
        if buffers[i] == 10:
            y=np.round(f10(temperature),2)
        if buffers[i] == 11:
            y=np.round(f11(temperature),2)
        ...
        for m in range(len(L)):
            ...
            if np.round(buffers[i]) == 9.0:
                EM9.append(ecart_moyenne)
            if np.round(buffers[i]) == 10.0:
                EM10.append(ecart_moyenne)
            if np.round(buffers[i]) == 11.0:
                EM11.append(ecart_moyenne)
    ...
    if res in ['O','o','y','Y']:
        if calib3:
            if buffer_value == '9':
                plot_calib(voltage_values, buffers, errorbuffers_values, errorvoltage_values, t, EM4, EM7, EM9, predict, equation, R2)
            if buffer_value == '10':
                plot_calib(voltage_values, buffers, errorbuffers_values, errorvoltage_values, t, EM4, EM7, EM10, predict, equation, R2)
            if buffer_value == '11':
                plot_calib(voltage_values, buffers, errorbuffers_values, errorvoltage_values, t, EM4, EM7, EM11, predict, equation, R2)
        else:
            plot_calib(voltage_values, buffers, errorbuffers_values, errorvoltage_values, t, EM4, EM7, predict, equation, R2)
    ...
    if r2 < 0.95:
        ...
        if x == '1':
            if calib3 :
                if buffer_value == '9':
                    Calibration(buffers = [7, 4, 9], n = 100)
                if buffer_value == '10':
                    Calibration(buffers = [7, 4, 10], n = 100)
                if buffer_value == '11':
                    Calibration(buffers = [7, 4, 11], n = 100)
            else :
                Calibration()
        if x == '2':
...
def Calibration_existante(buffer_existant):
    ...
    ################################################
    ...
    elif buffer_existant == '9':
        # liste des fichiers disponibles
        # comme la calib existante demande une calibration à 9 on ne fait que la liste des pH9 existants
        #
        cali_dispo = sorted(glob.glob('./CALIB/*pH9*.csv'), key=os.path.getmtime)
        print("Calibrations disponibles:")
        for i in range(len(cali_dispo)):
            print("%i - %s" % (i, cali_dispo[i]))
        res = input("Choisissez votre calibration en entrant son numéro d'ordre: ")
        ...
        ###############################
        csv_reader9 = csv.reader(c9, delimiter=';')
        L = []
        t = []
    ...
    ################################################
        ...
        # liste des fichiers disponibles
        # comme la calib existante demande une calibration à 11 on ne fait que la liste des pH11 existants
        #
        cali_dispo = sorted(glob.glob('./CALIB/*pH11*.csv'), key=os.path.getmtime)
        print("Calibrations disponibles:")
        for i in range(len(cali_dispo)):
            print("%i - %s" % (i, cali_dispo[i]))
        res = input("Choisissez votre calibration en entrant son numéro d'ordre: ")
        ...
        c4 = open(x_calib_existante4, 'r')
        c7 = open(x_calib_existante7, 'r')
        c11 = open(x_calib_existante11, 'r')
        ...
        ###############################
        csv_reader11 = csv.reader(c11, delimiter=';')
        L = []
        t = []
        for row in csv_reader11:
            L.append(float(row[2]))
            t.append(float(row[0]))
